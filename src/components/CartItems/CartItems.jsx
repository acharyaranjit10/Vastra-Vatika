    ...
    <div className="mt-24 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      {all_product.map((e) => {
        if (cartItems[e.id] > 0) {
          return (
            <div key={e.id}>
              <div className="grid md:grid-cols-[0.5fr_2fr_1fr_1fr_1fr_1fr] items-center gap-x-6 py-4 text-gray-600 font-medium text-sm md:text-base">
                <img className="h-16 w-16 object-contain" src={e.image} alt={e.name} />
                <p>{e.name}</p>
                <p className="text-right">Rs.{e.new_price}</p>
                <div className="flex justify-center">
                  <button className="w-16 h-10 border-2 border-gray-300 bg-white text-center">{cartItems[e.id]}</button>
                </div>
